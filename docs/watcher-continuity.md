...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, a re-arm whose recovery cycle is slowed after confirmation and must still surface rather than read as a watcher that stayed live, the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy, the already-acknowledged confirmation no-op for a matching generation with its mismatched-generation, dead-pid, and lock-mismatch rejections preserved, and the manual-restart generation-churn contract that a handling successor never mints.
