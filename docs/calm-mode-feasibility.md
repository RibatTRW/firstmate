...
| `assistant-working-note` | Assistant text in an `AssistantMessageComponent` message the model did not end its response with, identified by its own `stopReason` of `toolUse`, or of `length` with tool calls present | A text block that reads as routine working narration is removed from the shallow presentation copy before layout, so a `toolUse` message carrying only narration occupies zero rows (verified on Pi 0.85.1); a substantive block stays visible because a tool call in the same message is not evidence that its text was disposable; a still-streaming `pending` message is never filtered, so narration is briefly visible before the marker flips. |
